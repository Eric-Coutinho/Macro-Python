...
def playMacro(movements):
    for movement in movements:
        ...
        for i, checkpoint in enumerate(checkpoints):
            ...
            elif 'keyboard' in checkpoint:
                keyboard_input = checkpoint['keyboard']
                print('keyboard_input: ', keyboard_input)

                keyboard_controller.press(keyboard_input)

                # if i > 0 and 'time_interval' in checkpoint:
                #     time_interval = checkpoint['time_interval']
                #     time.sleep(time_interval)

                keyboard_controller.release(keyboard_controller)
            

def on_press(key):
    global isRunning, movements, checkpoints
...
def on_release(key):
    ...
    if key != keyboard.Key.right:
        ...
        time_interval = current_time - last_press
        movement = {"keyboard": key, "time_interval": time_interval}
